/*
 *   SCOMVITERBI2 Communications Toolbox S-Function for Viterbi decoder (modified)
 *
 *    Copyright 1996-2004 The MathWorks, Inc.
 *    $Revision: 1.26.4.5 $  $Date: 2004/04/12 23:03:41 $
 */

#define S_FUNCTION_NAME scomviterbi2
...
enum {INPORT_DATA=0, INPORT_MSCALE, INPORT_RESET};
enum {OUTPORT_DATA=0};
...
enum {TRELLIS_U_NUMBITS_ARGC = 0,   /* number of input bits, k      */
      ...
      MSCALE_ARGC,                  /* metric scaling port          */ 
      NUM_ARGS}; 
...
#define MSCALE_ARG              (ssGetSFcnParam(S, MSCALE_ARGC))
...
{ 
    ...
    /* Inputs and Outputs */ 
    {
        ...
        const boolean_T mscalePort = (boolean_T)((int_T)(mxGetPr(MSCALE_ARG)[0]) != 0);
        int8_T numInports = rstPort ? 3 : 2;
        ...
        if (1/*mscalePort*/) {
            if(!ssSetInputPortDimensionInfo(S, INPORT_MSCALE, DYNAMIC_DIMENSION)) return;
            ssSetInputPortFrameData(        S, INPORT_MSCALE, FRAME_INHERITED);
            ssSetInputPortDirectFeedThrough(S, INPORT_MSCALE, 1);
            ssSetInputPortReusable(         S, INPORT_MSCALE, 1);
            ssSetInputPortComplexSignal(    S, INPORT_MSCALE, COMPLEX_NO);
            ssSetInputPortSampleTime(       S, INPORT_MSCALE, INHERITED_SAMPLE_TIME);
            ssSetInputPortRequiredContiguous(S, INPORT_MSCALE, 1);
	}
 ...
 */
    ...
    /* Get metric scaling values (even if not enabled) */
    real_T          *mscaleIn  = (real_T *) ssGetInputPortSignal(S,INPORT_MSCALE);
    ...
    for(ib = 0; ib < blockSize; ++ib){
        ...
        for(indx1=0; indx1<(1<<n); indx1++) {
		    ...
		    real_T mScaling = (!mscalePort) ? 1 : mscaleIn[cOffset+n-1-indx2];
                    if((temp&01) == 1) {
                        /* logical 1 maps to -1.0 */
                        pbmet[indx1] += mScaling*pow((thisBlockIn[n-1-indx2] + 1.0),2); 
...
static void mdlSetDefaultPortDimensionInfo(SimStruct *S)
{
    ...
    /* Set default input and output port dimensions */
    {
        ...
        boolean_T portDimsKnownMscale = true;
        DECL_AND_INIT_DIMSINFO(dInfo);
        DECL_AND_INIT_DIMSINFO(dInfoMscale);
        ...
        dInfoMscale.dims = dims;
        portDimsKnownMscale = GetDefaultInputDimsIfPortDimsUnknown(S,INPORT_MSCALE,
                                                             cNumBits, &dInfoMscale);
        if(!portDimsKnownMscale){
            mdlSetInputPortDimensionInfo(S, INPORT_MSCALE, &dInfoMscale);
        }
    }
}
