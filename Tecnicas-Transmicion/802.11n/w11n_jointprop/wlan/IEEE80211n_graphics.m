function IEEE80211n_graphics(s, ax, firstcall, params);
% IEEE80211N_GRAPHICS  Update MATLAB graphics for IEEE 802.11n simulation.
% ieee80211n_graphics(s, ax, firstcall, params) updates axes.
% 
...
if firstcall
    ...
    %%%% added to remove function handle warnings (should remove this)
...
% Pre-computation
link_delay = 128; %%34;
speceff = (52/48)*params.bitsPerSymbol(s.mode);  % spectral efficiency
...
d.bitrate = (params.NSS)*54*speceff/4.5;  % 802.11n rate
d.ber = calcBER(s.txbits, s.rxbits, params.NSS*params.bitsPerBlock(s.mode), link_delay);
...
min_len = min(length(x)-1,length(d.txbits));
y = zeros(1,length(x)-1);
y(1:min_len) = d.txbits(1:min_len);
